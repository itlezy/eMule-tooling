...
| `CI-015` | In Progress | `f6cc0f9` tests | REST stress rows now enforce the native JSON-envelope path and report content-type counts, timeout count, and native non-JSON response count. Release x64 REST live smoke passed with 9914 stress requests, 0 failures, 0 timeouts, and 0 non-JSON native REST responses. qBit/Torznab/legacy concurrent traffic and soak validation remain open. |
